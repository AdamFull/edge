...
#define USE_VALIDATION_LAYERS
    ...
    (defined(EDGE_VK_USE_GPU_ASSISTED_VALIDATION) ||                           \
     defined(EDGE_VK_USE_BEST_PRACTICES_VALIDATION) ||                         \
     defined(EDGE_VK_USE_SYNCHRONIZATION_VALIDATION))
...
static VKAPI_ATTR VkBool32 VKAPI_CALL debug_utils_messenger_cb(
  ...
  if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT) {
    EDGE_LOG_DEBUG("[DebugUtilsMessenger]: %d - %s: %s",
                   callback_data->messageIdNumber,
                   callback_data->pMessageIdName, callback_data->pMessage);
  } else if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT) {
    EDGE_LOG_INFO("[DebugUtilsMessenger]: %d - %s: %s",
                  callback_data->messageIdNumber, callback_data->pMessageIdName,
                  callback_data->pMessage);
  } else if (message_severity &
             VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) {
    EDGE_LOG_WARN("[DebugUtilsMessenger]: %d - %s: %s",
                  callback_data->messageIdNumber, callback_data->pMessageIdName,
                  callback_data->pMessage);
  } else if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) {
    EDGE_LOG_ERROR("[DebugUtilsMessenger]: %d - %s: %s",
                   callback_data->messageIdNumber,
                   callback_data->pMessageIdName, callback_data->pMessage);
  }
...
static VkExtent2D
choose_suitable_extent(VkExtent2D request_extent,
  ...
  if (request_extent.width <= 1 || request_extent.height <= 1) {
    EDGE_LOG_WARN(
        "Image extent %dx%d is not supported. Selecting available %dx%d.",
  ...
  request_extent.width =
      clamp(request_extent.width, surface_caps->minImageExtent.width,
            surface_caps->maxImageExtent.width);
  request_extent.height =
      clamp(request_extent.height, surface_caps->minImageExtent.height,
...
static bool is_surface_format_hdr(const VkSurfaceFormatKHR *format) {
  return is_hdr_format(format->format) &&
         is_hdr_color_space(format->colorSpace);
}
...
static bool pick_by_priority_list(const VkSurfaceFormatKHR *available_formats,
  ...
  for (u32 p = 0; p < priority_count; p++) {
    for (u32 i = 0; i < available_count; i++) {
      ...
      if (surface_format_equal(&available_formats[i], &priority_list[p],
                               true) ||
          surface_format_equal(&available_formats[i], &priority_list[p],
                               false)) {
        *out_format = available_formats[i];
        return true;
      }
...
static bool instance_init() {
  ...
  if (layer_count > 0) {
    available_layers =
        g_ctx.alloc->allocate_array<VkLayerProperties>(layer_count);
  ...
  VkResult result =
      vkCreateInstance(&instance_info, &g_ctx.vk_alloc, &g_ctx.inst);
  ...
  if (g_ctx.validation_enabled) {
    ...
    result = vkCreateDebugUtilsMessengerEXT(g_ctx.inst, &debug_info,
                                            &g_ctx.vk_alloc, &g_ctx.debug_msgr);
    if (result != VK_SUCCESS) {
...
static bool select_adapter() {
  ...
  for (i32 i = 0; i < adapter_count; i++) {
    ...
    if (adapter_score > best_score) {
      ...
      memcpy(g_ctx.enabled_extensions, enabled_extensions,
             enabled_extension_count * sizeof(const char *));
      g_ctx.enabled_extension_count = enabled_extension_count;

      memcpy(g_ctx.queue_families, queue_families,
...
static bool device_init() {
  ...
  VkResult result =
      vkCreateDevice(g_ctx.adapter, &create_info, &g_ctx.vk_alloc, &g_ctx.dev);
  if (result != VK_SUCCESS) {
    return false;
  }
...
static bool allocator_init() {
  ...
  if (context_is_extension_enabled(
...
bool context_init(const ContextCreateInfo *create_info) {
  ...
  result = vkCreateWin32SurfaceKHR(g_ctx.inst, &surface_create_info,
                                   &g_ctx.vk_alloc, &g_ctx.surf);
  if (result != VK_SUCCESS) {
    goto fatal_error;
  }
#elif defined(VK_USE_PLATFORM_XLIB_KHR)
  ...
  result = vkCreateAndroidSurfaceKHR(g_ctx.inst, &surface_create_info,
                                     &g_ctx.vk_alloc, &g_ctx.surf);
  if (result != VK_SUCCESS) {
    goto fatal_error;
  }
  ...
  g_ctx.surf_format_count =
      min(g_ctx.surf_format_count, GFX_SURFACE_FORMAT_MAX);

  result = vkGetPhysicalDeviceSurfaceFormatsKHR(
      g_ctx.adapter, g_ctx.surf, &g_ctx.surf_format_count, g_ctx.surf_formats);
  if (result != VK_SUCCESS) {
    goto fatal_error;
  }
  ...
  g_ctx.surf_present_mode_count =
      min(g_ctx.surf_present_mode_count, GFX_PRESENT_MODES_MAX);

  result = vkGetPhysicalDeviceSurfacePresentModesKHR(
      g_ctx.adapter, g_ctx.surf, &g_ctx.surf_present_mode_count,
      g_ctx.surf_present_modes);
  if (result != VK_SUCCESS) {
    goto fatal_error;
  }
...
void context_shutdown() {
  ...
  if (g_ctx.debug_msgr != VK_NULL_HANDLE) {
    vkDestroyDebugUtilsMessengerEXT(g_ctx.inst, g_ctx.debug_msgr,
                                    &g_ctx.vk_alloc);
  }
...
static i32 queue_calculate_family_score(QueueRequest request,
  ...
  if (g_ctx.surf) {
    VkBool32 surface_supported = VK_FALSE;
    vkGetPhysicalDeviceSurfaceSupportKHR(g_ctx.adapter, family_index,
                                         g_ctx.surf, &surface_supported);
...
bool Queue::submit(Fence fence, const VkSubmitInfo2KHR *submit_info) {
  ...
  return vkQueueSubmit2KHR(queue_, 1, submit_info,
  ...
  case VK_QUERY_TYPE_OCCLUSION: {
    result =
        vkGetQueryPoolResults(g_ctx.dev, handle, first_query, 1, sizeof(u64),
                              out_data, sizeof(u64), VK_QUERY_RESULT_64_BIT);
    break;
  }
  case VK_QUERY_TYPE_TIMESTAMP: {
    result = vkGetQueryPoolResults(g_ctx.dev, handle, first_query * 2, 2,
                                   sizeof(u64) * 2, out_data, sizeof(u64),
...
bool PipelineCache::create(const u8 *data, usize data_size) {
  ...
  return vkCreatePipelineCache(g_ctx.dev, &create_info, &g_ctx.vk_alloc,
                               &handle) == VK_SUCCESS;
}
...
bool ShaderModule::create(const u32 *code, usize size) {
  ...
  return vkCreateShaderModule(g_ctx.dev, &create_info, &g_ctx.vk_alloc,
                              &handle) == VK_SUCCESS;
}
...
bool Swapchain::create(SwapchainCreateInfo create_info) {
  ...
  const VkSwapchainCreateInfoKHR swapchain_create_info = {
      ...
      .imageSharingMode = g_ctx.queue_family_count > 1
  ...
  result = vkCreateSwapchainKHR(g_ctx.dev, &swapchain_create_info,
                                &g_ctx.vk_alloc, &handle);
  if (result != VK_SUCCESS) {
    return false;
  }
...
bool Swapchain::update() {
  ...
  const VkSwapchainCreateInfoKHR create_info = {
      ...
      .imageExtent =
          choose_suitable_extent(surf_caps.currentExtent, &surf_caps),
      .imageArrayLayers = 1,
      .imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
      .imageSharingMode = g_ctx.queue_family_count > 1
                              ? VK_SHARING_MODE_CONCURRENT
  ...
  result =
      vkCreateSwapchainKHR(g_ctx.dev, &create_info, &g_ctx.vk_alloc, &handle);
  if (result != VK_SUCCESS) {
    return false;
  }
...
bool Swapchain::is_outdated() {
  ...
  if (surf_caps.currentExtent.width == 0xFFFFFFFF ||
      surf_caps.currentExtent.height == 0xFFFFFFFF) {
    return false;
  }

  return extent.width != surf_caps.currentExtent.width ||
...
bool Swapchain::acquire_next_image(u64 timeout, Semaphore semaphore,
  ...
  return vkAcquireNextImageKHR(g_ctx.dev, handle, timeout, semaphore,
                               VK_NULL_HANDLE, next_image_idx) == VK_SUCCESS;
}
...
bool Image::create(ImageCreateInfo create_info) {
  ...
  VmaAllocationCreateInfo allocation_create_info = {.usage =
  ...
  const VkImageCreateInfo image_create_info = {
      ...
      .arrayLayers = clamp(create_info.layer_count * create_info.face_count, 1u,
                           g_ctx.properties.limits.maxImageArrayLayers),
      .samples = VK_SAMPLE_COUNT_1_BIT,
      .tiling = VK_IMAGE_TILING_OPTIMAL,
      .usage = create_info.usage_flags,
      .sharingMode = g_ctx.queue_family_count > 1 ? VK_SHARING_MODE_CONCURRENT
                                                  : VK_SHARING_MODE_EXCLUSIVE,
      .queueFamilyIndexCount = g_ctx.queue_family_count,
      .pQueueFamilyIndices = queue_family_indices,
...
bool Buffer::create(BufferCreateInfo create_info) {
  ...
  if (create_info.flags & BUFFER_FLAG_UNIFORM) {
    ...
    minimal_alignment =
        lcm(g_ctx.properties.limits.minUniformBufferOffsetAlignment,
            g_ctx.properties.limits.nonCoherentAtomSize);
...
static VulkanStateInfo get_image_state_info(ResourceState state) {
  ...
  case ResourceState::TransferSrc:
    return VulkanStateInfo{.layout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                           .stage_mask = VK_PIPELINE_STAGE_2_TRANSFER_BIT,
                           .access_mask = VK_ACCESS_2_TRANSFER_READ_BIT};
  case ResourceState::TransferDst:
    return VulkanStateInfo{.layout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                           .stage_mask = VK_PIPELINE_STAGE_2_TRANSFER_BIT,
                           .access_mask = VK_ACCESS_2_TRANSFER_WRITE_BIT};
  case ResourceState::ShaderReadOnly:
    return VulkanStateInfo{.layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                           .stage_mask =
  ...
  case ResourceState::ShaderReadWrite:
    return VulkanStateInfo{.layout = VK_IMAGE_LAYOUT_GENERAL,
                           .stage_mask =
                               VK_PIPELINE_STAGE_2_VERTEX_SHADER_BIT |
                               VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT |
                               VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
  ...
  case ResourceState::Present:
    return VulkanStateInfo{.layout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
                           .stage_mask = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
                           .access_mask = VK_ACCESS_2_NONE};
...
static VulkanStateInfo get_buffer_state_info(ResourceState state) {
  ...
  case ResourceState::Undefined:
    return VulkanStateInfo{.stage_mask = VK_PIPELINE_STAGE_2_NONE,
                           .access_mask = VK_ACCESS_2_NONE};
  case ResourceState::TransferSrc:
    return VulkanStateInfo{.stage_mask = VK_PIPELINE_STAGE_2_TRANSFER_BIT,
                           .access_mask = VK_ACCESS_2_TRANSFER_READ_BIT};
  case ResourceState::TransferDst:
    return VulkanStateInfo{.stage_mask = VK_PIPELINE_STAGE_2_TRANSFER_BIT,
                           .access_mask = VK_ACCESS_2_TRANSFER_WRITE_BIT};
  case ResourceState::ShaderReadOnly:
    return VulkanStateInfo{.stage_mask =
                               VK_PIPELINE_STAGE_2_VERTEX_SHADER_BIT |
                               VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT |
                               VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
                           .access_mask = VK_ACCESS_2_SHADER_STORAGE_READ_BIT};
  case ResourceState::ShaderReadWrite:
    return VulkanStateInfo{.stage_mask =
                           ...
                           .access_mask = VK_ACCESS_2_SHADER_STORAGE_READ_BIT |
                                          VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT};
  case ResourceState::General:
    return VulkanStateInfo{.stage_mask = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,
                           .access_mask = VK_ACCESS_2_MEMORY_READ_BIT |
                                          VK_ACCESS_2_MEMORY_WRITE_BIT};
  case ResourceState::VertexBuffer:
    return VulkanStateInfo{.stage_mask = VK_PIPELINE_STAGE_2_VERTEX_INPUT_BIT,
                           .access_mask =
                               VK_ACCESS_2_VERTEX_ATTRIBUTE_READ_BIT};
  case ResourceState::IndexBuffer:
    return VulkanStateInfo{.stage_mask = VK_PIPELINE_STAGE_2_INDEX_INPUT_BIT,
                           .access_mask = VK_ACCESS_2_INDEX_READ_BIT};
  case ResourceState::IndirectBuffer:
    return VulkanStateInfo{.stage_mask = VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT,
                           .access_mask =
  ...
  case ResourceState::HostRead:
    return VulkanStateInfo{.stage_mask = VK_PIPELINE_STAGE_2_HOST_BIT,
                           .access_mask = VK_ACCESS_2_HOST_READ_BIT};
  case ResourceState::HostWrite:
    return VulkanStateInfo{.stage_mask = VK_PIPELINE_STAGE_2_HOST_BIT,
                           .access_mask = VK_ACCESS_2_HOST_WRITE_BIT};
...
bool Sampler::create(const VkSamplerCreateInfo &create_info) {
  return vkCreateSampler(g_ctx.dev, &create_info, &g_ctx.vk_alloc, &handle) ==
         VK_SUCCESS;
}
