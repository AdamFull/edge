...
enum class RenderResourceType { None = 0, Image, Buffer, Sampler };
...
struct RenderResource {
  using ResourceType = std::variant<std::monostate, ImageResource,
                                    BufferResource, SamplerResource>;

  ResourceType resource;
  ResourceState state = ResourceState::Undefined;

  u32 srv_index = ~0u;
  u32 uav_indices[RENDERER_UAV_MAX] = {~0u, ~0u, ~0u, ~0u, ~0u, ~0u, ~0u, ~0u,
  ...
  bool is_image() const {
    return std::holds_alternative<ImageResource>(resource);
  }
  bool is_buffer() const {
    return std::holds_alternative<BufferResource>(resource);
  }
  bool is_sampler() const {
    return std::holds_alternative<SamplerResource>(resource);
  }
  ...
  template <typename T> T *as() { return std::get_if<T>(&resource); }
...
struct RendererFrame {
  ...
  BufferView try_allocate_staging_memory(NotNull<const Allocator *> alloc,
...
struct BufferUpdateInfo {
  ...
  bool write(NotNull<const Allocator *> alloc, Span<const u8> data,
...
struct ImageUpdateInfo {
  ...
  bool write(NotNull<const Allocator *> alloc,
...
struct Renderer {
  ...
  StateTranslation
      state_translations[IMAGE_BARRIERS_MAX + BUFFER_BARRIERS_MAX] = {};
