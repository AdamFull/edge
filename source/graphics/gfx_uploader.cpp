...
ResourceSet::try_allocate_staging_memory(NotNull<const Allocator *> alloc,
  ...
  return BufferView{.buffer = staging_memory,
                    .local_offset = staging_offset.fetch_add(
                        aligned_requested_size, std::memory_order_acq_rel),
                    .size = aligned_requested_size};
}
...
bool Uploader::create(NotNull<const Allocator *> alloc,
  ...
  if (thread_create(&thread_handle, Uploader::thread_entry, this) !=
...
ImagePromise *Uploader::load_image(NotNull<const Allocator *> alloc,
  ...
  upload_commands.enqueue({.type = UploadingCommandType::Image,
                           .path = path,
...
ResourceSet &Uploader::get_resource_set() {
  return resource_sets[resource_set_index.load(std::memory_order_relaxed) %
                       FRAME_OVERLAP];
}
...
void Uploader::load_image_job(NotNull<const Allocator *> alloc,
  ...
  const ImageCreateInfo create_info = {
      .extent = {image_info.base_width, image_info.base_height,
                 image_info.base_depth},
  ...
  barrier_builder.add_image(image, ResourceState::Undefined,
  ...
  if (!buffer_view) {
    EDGE_LOG_ERROR(
        "Image loading failed. Failed to allocate uploading memory.");
  ...
  while (reader->read_next_block(buffer_dst, copy_offset, read_block_info) !=
         IImageReader::Result::EndOfStream) {
    copy_regions.push_back(
        ...
        {.sType = VK_STRUCTURE_TYPE_BUFFER_IMAGE_COPY_2_KHR,
         ...
         .imageOffset = {},
         .imageExtent = {.width = read_block_info.block_width,
...
i32 Uploader::thread_loop() {
  ...
  while (!should_exit.load(std::memory_order_acquire)) {
    ...
    while (upload_commands.dequeue(&command)) {
      ...
      Job *job =
          Job::from_lambda(allocator, sched, [this, path = command.path]() {
