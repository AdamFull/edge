...
constexpr BufferFlags k_vertex_buffer_flags =
    BUFFER_FLAG_DYNAMIC | BUFFER_FLAG_DEVICE_ADDRESS | BUFFER_FLAG_VERTEX;
constexpr BufferFlags k_index_buffer_flags =
    BUFFER_FLAG_DYNAMIC | BUFFER_FLAG_DEVICE_ADDRESS | BUFFER_FLAG_INDEX;
...
bool ImGuiRenderer::create(NotNull<const Allocator *> alloc,
  ...
  VkViewport viewport_state = {0.0f, 0.0f, 1280.0f, 720.0f, 0.0f, 1.0f};

  VkRect2D scissor_rect = {.offset = {0, 0}, .extent = {1280, 720}};
  ...
  vertex_buffer = renderer->create_empty();
  vertex_buffer_capacity = k_initial_vertex_count;

  index_buffer = renderer->create_empty();
...
void ImGuiRenderer::execute(NotNull<const Allocator *> alloc) {
  ...
  RenderResource *bb_rres =
      renderer->get_resource(renderer->get_backbuffer_handle());
  ...
  if (bb_rres->state != ResourceState::ColorAttachment) {
    renderer->add_state_translation(renderer->get_backbuffer_handle(),
  ...
  const VkRenderingInfoKHR rendering_info = {
      ...
      .renderArea = {.offset = {0, 0},
                     .extent = {bb_res->handle.extent.width,
                                bb_res->handle.extent.height}},
      .layerCount = 1,
  ...
  cmd.bind_index_buffer(ib_res->handle, sizeof(ImDrawIdx) == 2
                                            ? VK_INDEX_TYPE_UINT16
                                            : VK_INDEX_TYPE_UINT32);
  cmd.bind_pipeline(pipeline);

  cmd.set_viewport(0.0f, 0.0f, (f32)bb_res->handle.extent.width,
                   (f32)bb_res->handle.extent.height);
  ...
  imgui::PushConstant push_constant = {
      .vertices = vb_res->handle.address,
      .scale = {2.0f / draw_data->DisplaySize.x,
                2.0f / draw_data->DisplaySize.y},
      .translate = {-1.0f - draw_data->DisplayPos.x * push_constant.scale.x,
                    -1.0f - draw_data->DisplayPos.y * push_constant.scale.y},
      .sampler_index = 0};

  ImTextureBinding last_image_binding =
      ImTextureBinding{HANDLE_INVALID, HANDLE_INVALID};
  ...
  for (i32 n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    for (i32 cmd_i = 0; cmd_i < im_cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (clip_max.x > io.DisplaySize.x) {
        clip_max.x = io.DisplaySize.x;
      }
      if (clip_max.y > io.DisplaySize.y) {
        clip_max.y = io.DisplaySize.y;
      }
      if (clip_max.x <= clip_min.x || clip_max.y <= clip_min.y)
        continue;

      // Apply scissor/clipping rectangle
      cmd.set_scissor(clip_min.x, clip_min.y, clip_max.x - clip_min.x,
                      clip_max.y - clip_min.y);

      ImTextureBinding new_image_binding =
          ImTextureBinding::from_texture_id(pcmd->GetTexID());
      if (new_image_binding != last_image_binding) {
        RenderResource *image_resource =
            renderer->get_resource(new_image_binding.image);
        push_constant.image_index = image_resource->srv_index;
        RenderResource *sampler_resource =
            renderer->get_resource(new_image_binding.sampler);
        push_constant.sampler_index =
            sampler_resource ? sampler_resource->srv_index : 0u;
        // TODO: Add sampler
        renderer->push_constants(VK_SHADER_STAGE_ALL_GRAPHICS |
                                     VK_SHADER_STAGE_COMPUTE_BIT,
                                 push_constant);
        last_image_binding = new_image_binding;
      }

      cmd.draw_indexed(pcmd->ElemCount, 1u, pcmd->IdxOffset + global_idx_offset,
                       pcmd->VtxOffset + global_vtx_offset, 0u);
    }
...
void ImGuiRenderer::update_buffers(NotNull<const Allocator *> alloc) {
  if (vertex_need_to_grow) {
    BufferCreateInfo create_info = {.size = vertex_buffer_capacity *
                                            sizeof(ImDrawVert),
    ...
    renderer->update_buffer(alloc, vertex_buffer, buffer);
    vertex_need_to_grow = false;
  }

  if (index_need_to_grow) {
    BufferCreateInfo create_info = {.size = index_buffer_capacity *
                                            sizeof(ImDrawIdx),
    ...
    renderer->update_buffer(alloc, index_buffer, buffer);
    index_need_to_grow = false;
  }
}

void ImGuiRenderer::update_texture(NotNull<const Allocator *> alloc,
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ImageCreateInfo create_info = {.extent = {.width = (u32)tex->Width,
    ...
    Handle image_handle = renderer->create_image(create_info);
    if (image_handle.is_invalid()) {
    ...
    renderer->add_state_translation(image_handle, ResourceState::TransferDst);
    renderer->translate_states(cmd);
    ...
    ImageUpdateInfo update_info = {
        .dst_image = *image,
        .buffer_view = renderer->active_frame->try_allocate_staging_memory(
            alloc, whole_size, 1)};

    update_info.write(alloc, {.data = {tex->Pixels, whole_size},
                              .extent = {.width = (u32)tex->Width,
                                         .height = (u32)tex->Height,
                                         .depth = 1}});
    renderer->image_update_end(alloc, update_info);

    renderer->add_state_translation(image_handle,
    ...
    Image *image = resource->as_image();

    usize total_size = 0;
    for (const ImTextureRect &update_region : tex->Updates) {
      EDGE_LOG_DEBUG("Updating image %" PRIu64 " region: [%" PRIu16 ", %" PRIu16
                     ", %" PRIu16 ", %" PRIu16 "]",
                     tex->GetTexID(), update_region.x, update_region.y,
    ...
    renderer->add_state_translation(resource_id, ResourceState::TransferDst);
    renderer->translate_states(cmd);
    ...
    for (const ImTextureRect &update_region : tex->Updates) {
      ...
      for (usize y = 0; y < update_region.h; y++) {
        const void *src_pixels =
            tex->GetPixelsAt(update_region.x, update_region.y + y);
        memcpy(compacted_data + buffer_offset + (region_pitch * y), src_pixels,
               region_pitch);
      }
      ...
      update_info.write(
          ...
          {.data = {compacted_data + buffer_offset, region_size},
           .offset = {.x = update_region.x, .y = update_region.y, .z = 0},
           .extent = {.width = update_region.w,
...
void ImGuiRenderer::update_geometry(NotNull<const Allocator *> alloc,
  ...
  BufferUpdateInfo vb_update = {
      .dst_buffer = vb_res->handle,
      .buffer_view = renderer->active_frame->try_allocate_staging_memory(
          alloc, draw_data->TotalVtxCount * sizeof(ImDrawVert), 1)};

  BufferUpdateInfo ib_update = {
      .dst_buffer = ib_res->handle,
      .buffer_view = renderer->active_frame->try_allocate_staging_memory(
          alloc, draw_data->TotalIdxCount * sizeof(ImDrawIdx), 1)};
  ...
  for (i32 n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    vb_update.write(alloc, {(u8 *)im_cmd_list->VtxBuffer.Data, vtx_size},
                    std::exchange(vtx_offset, vtx_offset + vtx_size));

    auto idx_size = im_cmd_list->IdxBuffer.Size * sizeof(ImDrawIdx);
    ib_update.write(alloc, {(u8 *)im_cmd_list->IdxBuffer.Data, idx_size},
                    std::exchange(idx_offset, idx_offset + idx_size));
  }

  CmdBuf cmd = renderer->active_frame->cmd;

  renderer->add_state_translation(vertex_buffer, ResourceState::TransferDst);
  renderer->add_state_translation(index_buffer, ResourceState::TransferDst);
  renderer->translate_states(cmd);

  renderer->buffer_update_end(alloc, vb_update);
  renderer->buffer_update_end(alloc, ib_update);

  renderer->add_state_translation(vertex_buffer, ResourceState::VertexBuffer);
  renderer->add_state_translation(index_buffer, ResourceState::IndexBuffer);
  renderer->translate_states(cmd);
}
} // namespace edge::gfx