...
bool RendererFrame::create(NotNull<const Allocator *> alloc, CmdPool cmd_pool) {
  BufferCreateInfo buffer_create_info = {.size =
...
void RendererFrame::destroy(NotNull<const Allocator *> alloc,
...
BufferView
RendererFrame::try_allocate_staging_memory(NotNull<const Allocator *> alloc,
...
bool BufferUpdateInfo::write(NotNull<const Allocator *> alloc,
                             Span<const u8> data, VkDeviceSize dst_offset) {
...
bool ImageUpdateInfo::write(NotNull<const Allocator *> alloc,
...
bool Renderer::create(NotNull<const Allocator *> alloc,
  ...
  for (usize i = 0; i < swapchain.image_count; ++i) {
    VkImageSubresourceRange subresource_range = {.aspectMask =
...
bool Renderer::attach_image(Handle h, Image img) {
  ...
  VkImageAspectFlags image_aspect =
      (img.usage_flags & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT)
  ...
  if (img.extent.depth > 1) {
    ...
    if (img.layer_count > 1) {
      view_type = (img.face_count > 1) ? VK_IMAGE_VIEW_TYPE_CUBE_ARRAY
                                       : VK_IMAGE_VIEW_TYPE_2D_ARRAY;
    } else {
      view_type = (img.face_count > 1) ? VK_IMAGE_VIEW_TYPE_CUBE
                                       : VK_IMAGE_VIEW_TYPE_2D;
    }
  } else if (img.extent.width > 1) {
    view_type = img.layer_count > 1 ? VK_IMAGE_VIEW_TYPE_1D_ARRAY
...
bool Renderer::update_image(NotNull<const Allocator *> alloc, Handle h,
                            Image img) {
...
bool Renderer::update_buffer(NotNull<const Allocator *> alloc, Handle h,
                             Buffer buf) {
  if (!resource_pool.is_valid(h)) {
    return false;
  }
...
bool Renderer::update_sampler(NotNull<const Allocator *> alloc, Handle h,
                              Sampler smp) {
  if (!resource_pool.is_valid(h)) {
    return false;
  }
    ...
    std::visit(
        [&](auto &&data) {
          ...
          if constexpr (std::is_same_v<T, ImageResource>) {
            ...
            builder.add_image(data.handle, res->state, translation.new_state,
                              subresource_range);
          } else if constexpr (std::is_same_v<T, BufferResource>) {
            builder.add_buffer(data.handle, res->state, translation.new_state,
                               0, VK_WHOLE_SIZE);
          }
...
bool Renderer::frame_end(NotNull<const Allocator *> alloc,
  ...
  if (!write_descriptor_sets.empty()) {
    updete_descriptors(write_descriptor_sets.data(),
...
void Renderer::image_update_end(NotNull<const Allocator *> alloc,
...
void Renderer::buffer_update_end(NotNull<const Allocator *> alloc,
