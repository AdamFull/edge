...
PadBtn paddleboat_button_to_engine_btn(Paddleboat_Buttons button) {
  switch (button) {
  case PADDLEBOAT_BUTTON_A:
    return PadBtn::A;
  case PADDLEBOAT_BUTTON_B:
    return PadBtn::B;
  case PADDLEBOAT_BUTTON_X:
    return PadBtn::X;
  case PADDLEBOAT_BUTTON_Y:
    return PadBtn::Y;
  case PADDLEBOAT_BUTTON_L1:
    return PadBtn::BumperLeft;
  case PADDLEBOAT_BUTTON_L2:
    return PadBtn::TriggerLeft;
  case PADDLEBOAT_BUTTON_R1:
    return PadBtn::BumperRight;
  case PADDLEBOAT_BUTTON_R2:
    return PadBtn::TriggerRight;
  case PADDLEBOAT_BUTTON_SELECT:
    return PadBtn::Back;
  case PADDLEBOAT_BUTTON_START:
    return PadBtn::Start;
  case PADDLEBOAT_BUTTON_SYSTEM:
    return PadBtn::Guide;
  case PADDLEBOAT_BUTTON_L3:
    return PadBtn::ThumbLeft;
  case PADDLEBOAT_BUTTON_R3:
    return PadBtn::ThumbRight;
  case PADDLEBOAT_BUTTON_DPAD_UP:
    return PadBtn::DpadUp;
  case PADDLEBOAT_BUTTON_DPAD_RIGHT:
    return PadBtn::DpadRight;
  case PADDLEBOAT_BUTTON_DPAD_DOWN:
    return PadBtn::DpadDown;
  case PADDLEBOAT_BUTTON_DPAD_LEFT:
...
MouseBtn
paddleboat_mouse_button_to_engine_btn(Paddleboat_Mouse_Buttons button) {
  switch (button) {
  case PADDLEBOAT_MOUSE_BUTTON_LEFT:
    return MouseBtn::Left;
  case PADDLEBOAT_MOUSE_BUTTON_RIGHT:
    return MouseBtn::Right;
  case PADDLEBOAT_MOUSE_BUTTON_MIDDLE:
    return MouseBtn::Middle;
  case PADDLEBOAT_MOUSE_BUTTON_BACK:
    return MouseBtn::_4;
  case PADDLEBOAT_MOUSE_BUTTON_FORWARD:
    return MouseBtn::_5;
  default:
...
struct AndroidRuntime final : IRuntime {
  bool init(const RuntimeInitInfo &init_info) override {
    ...
    Paddleboat_ErrorCode result =
        Paddleboat_init(jni_env, layout->app->activity->javaGameActivity);
  ...
  bool requested_close() const override { return should_close; }
  ...
  void process_events() override {
    ...
    struct android_input_buffer *input_buf =
        android_app_swap_input_buffers(app);
    if (!input_buf) {
      return;
    }
    ...
    if (input_buf->motionEventsCount) {
      for (i32 idx = 0; idx < input_buf->motionEventsCount; idx++) {
        ...
        if (Paddleboat_processGameActivityMotionInputEvent(
                event, sizeof(GameActivityMotionEvent)) != 0) {
          continue;
        }
    ...
    if (input_buf->keyEventsCount) {
      for (i32 idx = 0; idx < input_buf->keyEventsCount; idx++) {
        ...
        if (Paddleboat_processGameActivityKeyInputEvent(
                event, sizeof(GameActivityKeyEvent)) != 0) {
          return;
        }
    ...
    if (Paddleboat_getMouseData(&mouse_data) == PADDLEBOAT_NO_ERROR) {
      mouse->set_axis(MouseAxis::Pos,
                      {mouse_data.mouseX, mouse_data.mouseY, 0.0f});
      mouse->set_axis(MouseAxis::Scroll,
                      {(f32)mouse_data.mouseScrollDeltaH,
                       (f32)mouse_data.mouseScrollDeltaV, 0.0f});

      // Process mouse buttons
      for (i32 button_idx = 0; button_idx < 8; ++button_idx) {
        auto paddle_button = (Paddleboat_Mouse_Buttons)(1 << button_idx);
        MouseBtn engine_btn =
            paddleboat_mouse_button_to_engine_btn(paddle_button);
    ...
    for (i32 controller_idx = 0; controller_idx < PADDLEBOAT_MAX_CONTROLLERS;
         ++controller_idx) {
      ...
      Paddleboat_ControllerStatus status =
          Paddleboat_getControllerStatus(controller_idx);
      ...
      if (status == PADDLEBOAT_CONTROLLER_ACTIVE) {
        if (!pad->connected) {
          ...
          if (Paddleboat_getControllerInfo(controller_idx, &info) ==
              PADDLEBOAT_NO_ERROR) {
        ...
        if (Paddleboat_getControllerData(controller_idx, &controller_data) ==
            PADDLEBOAT_NO_ERROR) {
          // Update buttons
          for (i32 button_idx = 0; button_idx < PADDLEBOAT_BUTTON_COUNT;
               ++button_idx) {
          ...
          f32 left_x = apply_deadzone(controller_data.leftStick.stickX,
                                      pad->stick_deadzone);
          f32 left_y = apply_deadzone(controller_data.leftStick.stickY,
                                      pad->stick_deadzone);
          f32 right_x = apply_deadzone(controller_data.rightStick.stickX,
                                       pad->stick_deadzone);
          f32 right_y = apply_deadzone(controller_data.rightStick.stickY,
                                       pad->stick_deadzone);

          // Paddleboat already normalizes triggers to 0-1
          f32 trigger_left =
              apply_deadzone(controller_data.triggerL2, pad->trigger_deadzone);
          f32 trigger_right =
              apply_deadzone(controller_data.triggerR2, pad->trigger_deadzone);
  ...
  [[nodiscard]] f32 get_surface_scale_factor() const override {
    return (f32)AConfiguration_getDensity(layout->app->config) /
           (f32)ACONFIGURATION_DENSITY_MEDIUM;
  }
...
void on_app_cmd_cb(struct android_app *app, i32 cmd) {
  ...
  default: {
    const char *cmd_lut[21] = {"UNUSED_APP_CMD_INPUT_CHANGED",
                               "APP_CMD_INIT_WINDOW",
                               "APP_CMD_TERM_WINDOW",
...
extern "C" void android_main(struct android_app *state) {
  edge::RuntimeLayout runtime_layout = {.app = state};
