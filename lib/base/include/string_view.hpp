...
struct StringView {
  ...
  constexpr StringView(const CharT *str)
      : m_data{str}, m_length{str ? Traits::length(str) : 0} {}
  ...
  constexpr void remove_prefix(size_type count) {
    assert(m_length >= count &&
           "cannot remove_prefix() larger than StringView size");
    m_data += count;
    m_length -= count;
  }
  ...
  constexpr bool ends_with(StringView suffix) const {
    ...
    return Traits::compare(m_data + (m_length - suffix.m_length), suffix.m_data,
                           suffix.m_length) == 0;
  }
  ...
  constexpr bool contains(CharT c) const { return find(c) != SIZE_MAX; }
  ...
  constexpr const_iterator end() const { return m_data + m_length; }
...
constexpr bool operator==(StringView<CharT, Traits> lhs,
                          StringView<CharT, Traits> rhs) {
  return lhs.compare(rhs) == 0;
}

template <Character CharT, typename Traits>
struct Hash<StringView<CharT, Traits>> {
  EDGE_FORCE_INLINE usize
  operator()(const StringView<CharT, Traits> &sv) const {
#if EDGE_HAS_SSE4_2
