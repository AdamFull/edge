...
template <FloatingPoint T> constexpr T clamp01(T x) {
...
template <std::integral T> constexpr T lcm(T a, T b) {
  if (a == 0 || b == 0) {
    return 0;
  }
  if constexpr (std::is_signed_v<T>) {
...
template <SignedArithmetic T> constexpr T abs(T x) {
...
template <Arithmetic T> constexpr T sign(T x) {
  if constexpr (std::is_floating_point_v<T>) {
...
template <UnsignedArithmetic T> constexpr bool is_pow2(T x) {
  return x != 0 && (x & (x - 1)) == 0;
}

template <UnsignedArithmetic T> constexpr T next_pow2(T x) {
  if (x == 0) {
    return 1;
  }
  --x;
...
template <UnsignedArithmetic T> constexpr T prev_pow2(T x) {
  if (x == 0) {
    return 0;
  }
  x |= x >> 1;
...
template <UnsignedArithmetic T> constexpr i32 clz(T x) {
  if (x == 0) {
    return sizeof(T) * 8;
  }
  i32 count = 0;
...
template <UnsignedArithmetic T> constexpr i32 ctz(T x) {
  if (x == 0) {
    return sizeof(T) * 8;
  }
  i32 count = 0;
...
template <FloatingPoint T> constexpr T lerp(T a, T b, T t) {
  return a + (b - a) * t;
}

template <FloatingPoint T> constexpr T inv_lerp(T a, T b, T value) {
  return (value - a) / (b - a);
}
...
template <FloatingPoint T> constexpr T smoothstep(T edge0, T edge1, T x) {
  T t = clamp((x - edge0) / (edge1 - edge0), T(0), T(1));
  return t * t * (T(3) - T(2) * t);
}

template <FloatingPoint T> constexpr T smootherstep(T edge0, T edge1, T x) {
  T t = clamp((x - edge0) / (edge1 - edge0), T(0), T(1));
  return t * t * t * (t * (t * T(6) - T(15)) + T(10));
}

template <UnsignedArithmetic T> constexpr T align_up(T x, T align) {
  return (x + align - 1) & ~(align - 1);
}

template <UnsignedArithmetic T> constexpr T align_down(T x, T align) {
  return x & ~(align - 1);
}

template <Arithmetic T> constexpr T square(T x) { return x * x; }
...
template <FloatingPoint T> constexpr T radians(T degrees) {
...
template <FloatingPoint T> constexpr T degrees(T radians) {
  if constexpr (std::is_same_v<T, f32>) {
...
template <FloatingPoint T> constexpr bool approx_equal(T a, T b, T epsilon) {
  return abs(a - b) <= epsilon;
}

template <FloatingPoint T> constexpr bool approx_equal(T a, T b) {
  if constexpr (std::is_same_v<T, f32>) {
...
template <FloatingPoint T> constexpr T distance(T a, T b) { return abs(b - a); }

template <FloatingPoint T> constexpr T step(T edge, T x) {
  return x < edge ? T(0) : T(1);
}

template <FloatingPoint T> constexpr T ease_in_quad(T t) { return t * t; }

template <FloatingPoint T> constexpr T ease_out_quad(T t) {
  return t * (T(2) - t);
}

template <FloatingPoint T> constexpr T ease_in_out_quad(T t) {
  return t < T(0.5) ? T(2) * t * t : T(-1) + (T(4) - T(2) * t) * t;
}

template <FloatingPoint T> constexpr T ease_in_cubic(T t) { return t * t * t; }

template <FloatingPoint T> constexpr T ease_out_cubic(T t) {
  T t1 = t - T(1);
  return t1 * t1 * t1 + T(1);
}

template <FloatingPoint T> constexpr T ease_in_out_cubic(T t) {
  return t < T(0.5)
             ? T(4) * t * t * t
             : T(1) + (t - T(1)) * (T(2) * (t - T(1))) * (T(2) * (t - T(1)));
}

template <Arithmetic T> constexpr bool in_range(T x, T min_val, T max_val) {
