...
namespace edge {
template <TrivialType T> struct HeapStorage {
  T *m_data = nullptr;
  usize m_capacity = 0;
...
template <TrivialType T> inline T *begin(HeapStorage<T> &storage) {
  return storage.data();
}

template <TrivialType T> inline T *end(HeapStorage<T> &storage) {
  return storage.data() + storage.capacity();
}

template <TrivialType T> inline const T *begin(const HeapStorage<T> &storage) {
  return storage.data();
}

template <TrivialType T> inline const T *end(const HeapStorage<T> &storage) {
  return storage.data() + storage.capacity();
}

template <TrivialType T, usize N> struct StackStorage {
...
template <TrivialType T, usize N> inline T *begin(StackStorage<T, N> &storage) {
  return storage.data();
}

template <TrivialType T, usize N> inline T *end(StackStorage<T, N> &storage) {
  return storage.data() + storage.capacity();
}
...
template <TrivialType T> struct ExternalStorage {
  T *m_data = nullptr;
  usize m_capacity = 0;
...
template <TrivialType T> inline T *begin(ExternalStorage<T> &storage) {
  return storage.data();
}

template <TrivialType T> inline T *end(ExternalStorage<T> &storage) {
  return storage.data() + storage.capacity();
}
...
template <TrivialType T, typename StorageProvider> struct Buffer {
  ...
  usize capacity() const { return m_storage.capacity(); }
...
template <typename T> using HeapBuffer = Buffer<T, HeapStorage<T>>;
template <typename T, usize N>
using StackBuffer = Buffer<T, StackStorage<T, N>>;
template <typename T> using ExternalBuffer = Buffer<T, ExternalStorage<T>>;
