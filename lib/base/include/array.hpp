...
namespace edge {
	...
	template<TrivialType T, usize N>
	inline T* begin(FixedArray<T, N>& arr) {
		return arr.data();
	}

	template<TrivialType T, usize N>
	inline T* end(FixedArray<T, N>& arr) {
		return arr.data() + arr.size();
	}

	template<TrivialType T, usize N>
	inline const T* begin(const FixedArray<T, N>& arr) {
		return arr.data();
	}

	template<TrivialType T, usize N>
	inline const T* end(const FixedArray<T, N>& arr) {
		return arr.data() + arr.size();
	}
}
