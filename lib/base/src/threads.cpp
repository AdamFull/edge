...
ThreadResult mutex_unlock(Mutex *mtx) {
  ...
  return ReleaseMutex(mtx->handle) ? ThreadResult::Success
                                   : ThreadResult::Error;
}
...
ThreadResult thread_create(Thread *thr, ThreadFunc func, void *arg) {
  ...
  i32 result = pthread_create(reinterpret_cast<pthread_t *>(&thr->handle),
                              nullptr, detail::thread_start_wrapper, info);
  if (result != 0) {
    delete info;
...
i32 thread_sleep(const std::chrono::nanoseconds &duration) {
  ...
  auto nsecs =
      duration - std::chrono::duration_cast<std::chrono::nanoseconds>(secs);
...
static long futex_syscall(std::atomic<u32> *addr, i32 op, u32 val,
...
FutexResult futex_wait(std::atomic<u32> *addr, u32 expected_value,
  ...
  if (timeout != std::chrono::nanoseconds::zero()) {
    ...
    auto nsecs =
        timeout - std::chrono::duration_cast<std::chrono::nanoseconds>(secs);
...
ThreadResult mutex_init(Mutex *mtx, MutexType type) {
  ...
  i32 result = pthread_mutex_init(
      reinterpret_cast<pthread_mutex_t *>(&mtx->data), &attr);
...
ThreadResult mutex_lock(Mutex *mtx) {
  ...
  i32 result =
      pthread_mutex_lock(reinterpret_cast<pthread_mutex_t *>(&mtx->data));
  return result == 0 ? ThreadResult::Success : ThreadResult::Error;
}
...
ThreadResult mutex_trylock(Mutex *mtx) {
  ...
  i32 result =
      pthread_mutex_trylock(reinterpret_cast<pthread_mutex_t *>(&mtx->data));
  if (result == 0) {
    return ThreadResult::Success;
...
ThreadResult mutex_timedlock(Mutex *mtx,
  ...
  i32 result = pthread_mutex_timedlock(
...
ThreadResult mutex_unlock(Mutex *mtx) {
  ...
  i32 result =
      pthread_mutex_unlock(reinterpret_cast<pthread_mutex_t *>(&mtx->data));
  return result == 0 ? ThreadResult::Success : ThreadResult::Error;
}
...
ThreadResult cond_init(ConditionVariable *cnd) {
  ...
  i32 result = pthread_cond_init(reinterpret_cast<pthread_cond_t *>(&cnd->data),
                                 nullptr);
  return result == 0 ? ThreadResult::Success : ThreadResult::Error;
}
...
ThreadResult cond_signal(ConditionVariable *cnd) {
  ...
  i32 result =
      pthread_cond_signal(reinterpret_cast<pthread_cond_t *>(&cnd->data));
  return result == 0 ? ThreadResult::Success : ThreadResult::Error;
}
...
ThreadResult cond_broadcast(ConditionVariable *cnd) {
  ...
  i32 result =
      pthread_cond_broadcast(reinterpret_cast<pthread_cond_t *>(&cnd->data));
  return result == 0 ? ThreadResult::Success : ThreadResult::Error;
}
...
ThreadResult cond_wait(ConditionVariable *cnd, Mutex *mtx) {
  ...
  i32 result =
      pthread_cond_wait(reinterpret_cast<pthread_cond_t *>(&cnd->data),
...
ThreadResult cond_timedwait(ConditionVariable *cnd, Mutex *mtx,
  ...
  i32 result = pthread_cond_timedwait(
      reinterpret_cast<pthread_cond_t *>(&cnd->data),
      reinterpret_cast<pthread_mutex_t *>(&mtx->data), &ts);
  if (result == 0) {
    return ThreadResult::Success;
  } else if (result == ETIMEDOUT) {
...
ThreadResult thread_set_affinity(Thread &thr, i32 core_id,
  ...
  return thread_set_affinity_ex(thr, cpu_info, cpu_count, core_id,
                                prefer_physical);
}
} // namespace edge