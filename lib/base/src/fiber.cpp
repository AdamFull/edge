...
void __sanitizer_finish_switch_fiber(void *fake_stack_save,
                                     const void **bottom_old, usize *size_old);
}
#else
extern "C" {
void __sanitizer_start_switch_fiber(void **fake_stack_save, const void *bottom,
...
extern "C" void fiber_swap_context_internal(FiberContext *from,
                                            FiberContext *to);
...
FiberContext *fiber_context_create(const NotNull<const Allocator *> allocator,
  ...
  if (ctx->stack_ptr) {
    ...
    stack_top = reinterpret_cast<void *>(
        reinterpret_cast<uintptr_t>(stack_top) & ~0xFULL);
...
void fiber_context_destroy(NotNull<const Allocator *> allocator,
                           FiberContext *ctx) {
  if (!ctx) {
    return;
  }
...
void *fiber_get_stack_ptr(FiberContext *ctx) { return ctx->stack_ptr; }

usize fiber_get_stack_size(FiberContext *ctx) { return ctx->stack_size; }
...
bool fiber_context_switch(FiberContext *from, FiberContext *to) {
  ...
  __sanitizer_start_switch_fiber(
      &fake_stack, static_cast<char *>(to->stack_ptr), to->stack_size);
